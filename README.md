...
Dra inn blokken ``||Variables:set energi to||`` inn i løkken. Endre antall repetisjoner til hvor mange energi-spriter du vil ha på skjermen. Vi har valgt **80**
```blocks
namespace myTiles {
    //% blockIdentity=images._tile
    export const tile0 = img`
...
Plasser blokken inn i løkken. Endre type sprite til ``||variables:energi||`` og type **kart-flis** til lys brun (tilePath5).
```blocks
namespace myTiles {
    //% blockIdentity=images._tile
    export const tile0 = img`
...
```
## Steg 7: poengsum 
Hver gang ``||variables:mySprite||`` overlapper med ``||variables:energi||`` kan vi øke score-variabelen med 1. Blokken for det finner vi i ``||info:Info||``, ``||info:change score by||``. Plasser denne blokken i ``||sprites:overlapp-blokken||``.
```blocks
    sprites.onOverlap(SpriteKind.Player, SpriteKind.Food, function (sprite, otherSprite) {
    otherSprite.destroy()
...
```
## steg 8 set score til 0 ved start
Vi setter også poengsum til null ved start. Finn ``||info:set score to||`` fra ``||info:Info||``-menyen. 
Plasser denne nederst i koden i ``||loops:on start||``.
```blocks
namespace myTiles {
    //% blockIdentity=images._tile
    export const tile0 = img`
